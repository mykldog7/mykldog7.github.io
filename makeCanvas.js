...
function makeCanvasElements(parentELEM, imagesArray, imageParameters, imageList){
	if(imageList == undefined) {imageList = "image-";}
...
function addLablesToCanvases(parentELEM, labelArray, imageList){
	if(imageList == undefined) {imageList = "image-";}
	labelArray.forEach(function(label,index){
		var cElem = document.getElementById(imageList+index);
...
function addLablesToCanvasesExt(parentELEM, labelArray, offset, lblColor1, lblColor2, imageList){
	if(imageList == undefined) {imageList = "image-";}
	labelArray.forEach(function(label,index){
		var cElem = document.getElementById(imageList+(index+offset));
		var gcontext = cElem.getContext("2d");
		var xO = 21;		//postion and size of the small label...
		var yO = 18;
		var w = 7;
